...
# Protein Function Prediction Main Script
# Run Example:
#      python ENGINE_main.py --data_list ./infer_temp --cate cc

# Path to GO annotations
ANNOTATION_FILE = 'Dataset/nrPDB-GO_2019.06.18_annot.tsv'
# Number of GO terms per category
CLASS_DICT = {
    'mf':489,
    'bp':1943,
    'cc':320
...
# Loading GO annotation data
def load_GO_annot(filename):
    # Load GO annotations
    onts = ['mf', 'bp', 'cc']
    prot2annot = {}    # Mapping protein IDs to corresponding GO terms
...
# Dataset class for Protein Function Prediction using DSSP + ESM + 3Di
class PFresGODatasets(Dataset):
    def __init__(self,
                 pdb_root= 'infer_temp', # Input path for PDB data
                 category= 'cc' , # mf | cc | bp
                 esm3_6B_path = 'Features/esm-c', # Path to store extracted ESM-C features
                 feature_3Di_path ='Features/foldseek_3di_token' ,   # Path to store foldseek 3Di Tokens
                 dssp_executable = "dssp",  # DSSP installation path
                 foldseek_executable = "foldseek"  # Foldseek installation path
                 ):
        ...
        }                              # Mapping DSSP secondary structures to numeric vectors
        self.letter_to_num = {'C': 4, 'D': 3, 'S': 15, 'Q': 5, 'K': 11, 'I': 9,
                       'P': 14, 'T': 16, 'F': 13, 'A': 0, 'G': 7, 'H': 8,
                       'E': 6, 'L': 10, 'R': 1, 'W': 17, 'V': 19, 
                       'N': 2, 'Y': 18, 'M': 12,'X':20} #Defined according to the 3Di alphabet
        self.init_3Di()
    ...
    # Used to obtain information about the secondary structure of a protein, returning a coded secondary structure feature
    def get_dssp_ss_feature(self,pdb_file):
        parser = PDBParser(QUIET=True)
        structure = parser.get_structure('Protein', pdb_file)
        
        model = structure[0]  # Using the first model of the PDB
...
# Model inference and save result
def val(val_loader ,device, model ,CATE,save_path = ''):
    prot2annot, goterms, gonames, counts = load_GO_annot(ANNOTATION_FILE)
    goterms = goterms[CATE]
    gonames = gonames[CATE]
    with torch.no_grad():
        with open(os.path.join(save_path, 'infer_result.csv'), 'w') as f:
...
if __name__ == '__main__':
    ...
    # dssp as node into egnn, esm and token as sequence channel

    import torch
    device = 'cpu'
    # Argument parsing for testing

    pfres_config_parser = argparse.ArgumentParser('PFresGoDatasets')
    pfres_config_parser.add_argument('--data_list', type=str, default='infer_temp')
    pfres_config_parser.add_argument('--epoch', type=int, default=20)
    pfres_config_parser.add_argument('--dssp_executable', type=str, default='dssp') # DSSP installation path
    pfres_config_parser.add_argument('--foldseek_executable', type=str, default='foldseek') # foldseek installation path
    # pfres_config_parser.add_argument('--resume_path', type=str, default='')
    pfres_config_parser.add_argument('--cate', choices=['mf', 'bp', 'cc'], default='cc' ,help='choices the category of GO terms')

    pfres_config = pfres_config_parser.parse_args()
    data_list = pfres_config.data_list
    epoch = pfres_config.epoch
    # resume_path = pfres_config.resume_path
    CATE = pfres_config.cate

    # Specify different model paths for different categories
    if CATE == 'mf':
    ...
    test_dataset = PFresGODatasets(data_list, category=CATE, dssp_executable=pfres_config.dssp_executable, foldseek_executable=pfres_config.foldseek_executable)
    test_loader = DataLoader(test_dataset,batch_size=1)

    model = load_model(resume_path,device,cate = CATE)
    # Perform inference
    evaluation_file = val(val_loader=test_loader, device=device  , model = model,  CATE = CATE) 
